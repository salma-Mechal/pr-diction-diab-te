        ...
        try:
            ...
            if not os.path.exists(self.model_path):
                st.error(
                    ...
                    "Le fichier 'xgb_model.pkl' doit être placé dans le dossier 'models/'.\n\n"
                    "Solutions possibles :\n"
                    "1. Vérifiez que le fichier modèle existe bien\n"
                    "2. Téléchargez le modèle depuis son emplacement d'origine\n"
                    "3. Contactez l'administrateur pour obtenir le fichier modèle"
                )
                st.stop()
            ...
            logger.exception("Erreur de chargement du modèle")
            st.error(
                f"⚠️ Erreur technique lors du chargement du modèle :\n\n{str(e)}\n\n"
                "Causes possibles :\n"
                "1. Fichier modèle corrompu\n"
                "2. Version incompatible de scikit-learn/joblib\n"
...
def show_documentation_page() -> None:
    ...
    <div class="doc-section">
        ...
        <div class="doc-item">
            <strong>3. Modélisation</strong> : Implémentation de <strong>XGBoost</strong> avec :
            <ul style="margin-top: 0.5rem; margin-left: 1.5rem;">
                <li>Réglage des hyperparamètres</li>
                <li>la matrice de confusion</li>
                <li>la courbe AUC-ROC</li>
            </ul>
        </div>
    ...
    with st.expander("📁 Format du fichier CSV", expanded=False):
        ...
        <div class="doc-section">
            ...
            <div class="doc-item">• Pas de valeurs manquantes </div>
